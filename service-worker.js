...
const STATIC=['/manifest.webmanifest?v=254','/icon.svg?v=254','/badge.svg?v=254'];
...
self.addEventListener('push',event=>{let data={};try{data=event.data?event.data.json():{}}catch(_){data={body:event.data?event.data.text():'Nova mensagem recebida.'}}const title=data.title||'Perla Andina';const options={body:data.body||'Nova mensagem recebida.',icon:'/icon.svg?v=254',badge:'/badge.svg?v=254',tag:data.tag||'perla-b2b',renotify:true,requireInteraction:false,vibrate:[240,70,240,70,420],data:{openUrl:data.openUrl||'/menu',conversationId:data.conversationId||'',messageId:data.messageId||''}};event.waitUntil(self.registration.showNotification(title,options))});
