...
module.exports=async function handler(req,res){
  ...
  res.setHeader('Vercel-CDN-Cache-Control','public, max-age=120, stale-while-revalidate=600');
  ...
  try{
    const ctrl=new AbortController(),tm=setTimeout(()=>ctrl.abort(),45000);let r;
    try{
    ...
    if(!r.ok||!/<html[\s>]/i.test(html)||!/Perla Andina/i.test(html))return res.status(502).send('BACKEND_UI_INVALID');
